...
package net.mcreator.hantainotengoku.world.biome;
...
import net.mcreator.hantainotengoku.entity.VoidPeopleEntity;
import net.mcreator.hantainotengoku.entity.LowTierDemonEntity;
...
public class LimboBiomeBiome extends HantaiNoTengokuModElements.ModElement {
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == VoidSoilBlock.block.getDefaultState().getBlock() || ground == Blocks.STONE.getDefaultState().getBlock())
							&& (ground2 == VoidSoilBlock.block.getDefaultState().getBlock() || ground2 == Blocks.STONE.getDefaultState().getBlock())))
						return false;
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.OAK_LOG.getDefaultState().getBlock()
					|| blockType == Blocks.OAK_LEAVES.getDefaultState().getBlock() || blockType == VoidSoilBlock.block.getDefaultState().getBlock()
